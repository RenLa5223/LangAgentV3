...
async def build_chat_context(cfg: dict) -> tuple:
    ...
    user_profile_path = os.path.join(USER_PROFILE_DIR, "user_profile.txt")
    ...
    try:
        ...
        if rag_results:
            ...
            for res in rag_results:
                rag_context += f"-[{res['time']}] {res['content'][:400]}...\n"
...
async def call_llm_with_circuit_breaker(cfg: dict, messages: list, use_fallback: bool = True) -> str:
    ...
    if result and result.startswith(_MODEL_ERR):
        ...
        if fail_count >= 3:
            await state.set_api_cooldown(time.time() + 60)
...
def _sync_llm_call(cfg: dict, messages: list, use_fallback: bool) -> str:
    ...
    if api_format == "anthropic":
        ...
        payload = {"model": cfg['model'], "messages": anthropic_msgs, "max_tokens": 4096, "stream": False}
        if system_text:
